    ...
    <div className="w-full flex flex-col font-sans dir-rtl select-none">
      ...
      <div className="bg-white border border-gray-200 rounded-xl px-4 py-2.5 flex flex-col md:flex-row items-center justify-between gap-3 shadow-xs mt-6">
        ...
        <div className="flex items-center gap-2 shrink-0 text-xs font-bold">
          ...
          {/* DYNAMIC ACTION BUTTON: END OF MONTH (ONLY IF hasPendingEndOfMonth === true) */}
          {hasPendingEndOfMonth && (
            <button
              onClick={() => {
                alert('تمت معالجة إغلاق نهاية الشهر بنجاح!');
                setHasPendingEndOfMonth(false);
              }}
              className="bg-rose-600 hover:bg-rose-700 text-white font-black px-3 py-1 rounded-lg shadow-sm transition-all animate-bounce"
              title="انقر لإغلاق نهاية الشهر وإزالة الإشعار"
            >
              End of Month ⚠️
            </button>
          )}

          {/* DYNAMIC ACTION BUTTON: ALERTS (ONLY IF alertCount > 0) */}
          {alertCount > 0 && (
            <button
              onClick={() => {
                alert(`تم الاطلاع على ${alertCount} تنبيهات تشغيلية!`);
                setAlertCount(0);
              }}
              className="bg-rose-500 hover:bg-rose-600 text-white font-black px-2.5 py-1 rounded-lg shadow-sm transition-all flex items-center gap-1"
              title="انقر لتصفير الإشعارات والتنبيهات"
            >
          ...
          <div className="bg-gray-100 border border-gray-200 px-2.5 py-1 rounded-lg text-gray-800 flex items-center gap-1.5 font-mono">
            <span>USD rate: 90,000.00</span>
            <button
              onClick={() => onSelectScreen('sales-moresetup-currencysetup')}
              title="تعديل سعر الصرف الرسمي"
