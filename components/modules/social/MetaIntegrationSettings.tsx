  ...
  return (
    <div className="w-full bg-white rounded-2xl border border-slate-200 shadow-2xs p-5 space-y-6 font-sans text-slate-800 text-left select-none">
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-bold text-slate-800">
              {t('meta_business_manager_marketing_api', 'Meta Business Manager & Marketing API Configuration')}
            </h2>
          </div>
          <p className="text-xs text-slate-600 font-medium">
            {t('southern_olive_oil_products_sarl', 'Southern Olive Oil Products S.A.R.L - Connect central Ad Account, 13 Facebook Pages, Instagram profiles & WhatsApp Business')}
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          {connectionStatus === 'CONNECTED' && (
            <span className="px-3 py-1 bg-emerald-50 text-emerald-800 border border-emerald-200 rounded-full text-xs font-bold flex items-center gap-1.5">
              ...
              <span>{t('api_live_connected', 'API Live & Connected')}</span>
            </span>
          )}
          <button
            type="button"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
        <div>
          ...
          <input
            ...
            placeholder={t('act_1234567890', 'act_1234567890')}
            className="w-full px-3 py-2 border border-slate-300 rounded-lg font-mono text-xs font-bold text-primary focus:border-primary focus:outline-none"
          />
          <p className="text-[10.5px] text-slate-400 mt-0.5">{t('the_central_ad_account_linked_to_the', 'The central Ad Account linked to the company single payment card')}</p>
        </div>
        ...
        <div>
          ...
          />
          <p className="text-[10.5px] text-slate-400 mt-0.5">{t('connects_all_13_active_representative', 'Connects all 13 active representative WhatsApp lines')}</p>
        </div>

        <div>
          <label className="block font-bold text-slate-700 mb-1">{t('meta_app_id', 'Meta App ID')}</label>
          <input
            type="text"
            value={appId}
        ...
        <div>
          <label className="block font-bold text-slate-700 mb-1">{t('meta_app_secret', 'Meta App Secret')}</label>
          <input
            type="password"
            value={appSecret}
        ...
        <div className="md:col-span-2">
          ...
          <input
            ...
            className="w-full px-3 py-2 border border-slate-300 rounded-lg font-mono text-xs text-slate-800 focus:outline-none"
          />
          <p className="text-[10.5px] text-slate-400 mt-0.5">{t('permanent_nonexpiring_token_generated', 'Permanent non-expiring token generated from Meta Business Settings ➔ System Users')}</p>
        </div>
      </div>
      ...
      <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl flex flex-col md:flex-row md:items-center justify-between gap-3 text-xs">
        <div>
          <div className="font-bold text-slate-800">{t('live_metric_synchronization', 'Live Metric Synchronization:')}</div>
          <p className="text-[11px] text-slate-500">
            {t('pull_latest_ad_spend_leads_count', 'Pull latest ad spend, leads count, impressions, and calculate real CPL into the `Ad Campaigns & CPL` dashboard')}
          </p>
        ...
        <button
          ...
          className="px-5 py-2 bg-primary hover:bg-primary/90 text-primary-foreground font-bold rounded-lg shadow-sm transition-all whitespace-nowrap cursor-pointer"
        >
