    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white border border-slate-200 rounded-lg p-5 shadow-xs">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-bold text-slate-900">Grower &amp; Client Directory Ledgers</h2>
          </div>
          <p className="text-xs text-slate-500 mt-0.5">
            Farmer accounts, historical olive crushing intake, oil deposits stored in mill silos, and financial balances
          </p>
        </div>

        <button
          onClick={() => showToast('Exported grower directory statements.')}
        ...
        >
          ...
          <span>Export Directory Statements</span>
        </button>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-lg p-4 shadow-xs flex items-center justify-between gap-4">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            type="text"
            placeholder="Search farmer name, phone number, or grove location..."
            value={searchQuery}
        ...
        <span className="text-xs text-slate-500">{filteredAccounts.length} Registered Accounts</span>
      </div>
          ...
          <table className="w-full text-left text-xs text-slate-700 border-collapse">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50 text-[11px] font-bold text-slate-600 uppercase">
                <th className="py-2.5 px-3">Grower / Farmer Name</th>
                <th className="py-2.5 px-3">Contact Phone</th>
                <th className="py-2.5 px-3">Grove Location</th>
                <th className="py-2.5 px-3">Historical Crushed Olives</th>
                <th className="py-2.5 px-3">Oil Deposits in Silo</th>
                <th className="py-2.5 px-3">Available Tin Balance</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredAccounts.map((acc) => (
                <tr key={acc.id} className="hover:bg-slate-50">
                  ...
                  <td className="py-2.5 px-3 font-bold text-slate-800">{acc.tinBalanceAvailable} Tins</td>
                  ...
                  <td className="py-2.5 px-3 text-right">
                    <button
                      onClick={() => showToast(`Generated account statement for ${acc.farmerName}.`)}
