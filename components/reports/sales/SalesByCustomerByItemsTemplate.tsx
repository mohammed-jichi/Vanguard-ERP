...
export const SalesByCustomerByItemsTemplate: React.FC<SalesByCustomerByItemsTemplateProps> = ({
  ...
  const defaultItems = [
    { customer: 'Al-Baraka Supermarket S.A.R.L', itemCode: 'OIL-001', itemName: 'Extra Virgin Olive Oil 17.5L Standard Product', qty: 25, unitPrice: '$110.00', totalValue: '$2,750.00' },
    { customer: 'Al-Baraka Supermarket S.A.R.L', itemCode: 'OIL-002', itemName: 'Extra Virgin Olive Oil 1L Standard Product Standard Product', qty: 120, unitPrice: '$8.50', totalValue: '$1,020.00' },
    { customer: 'Karem Assaf Grocery', itemCode: 'OIL-001', itemName: 'Extra Virgin Olive Oil 17.5L Standard Product', qty: 15, unitPrice: '$110.00', totalValue: '$1,650.00' },
    { customer: 'Karem Assaf Grocery', itemCode: 'OIL-004', itemName: 'Molasses Pomegranate Molasses Local 500 ml', qty: 60, unitPrice: '$6.00', totalValue: '$360.00' },
    { customer: 'Ahmad Al-Hajj Wholesale', itemCode: 'OIL-001', itemName: 'Extra Virgin Olive Oil 17.5L Standard Product', qty: 40, unitPrice: '$110.00', totalValue: '$4,400.00' },
    { customer: 'Ahmad Al-Hajj Wholesale', itemCode: 'OIL-003', itemName: 'Standard Product Local gStandard Product Standard ProductOlive Oil KgStandard Product', qty: 30, unitPrice: '$18.00', totalValue: '$540.00' },
  ];
      ...
      {isNotSold ? (
        ...
        <div className="border border-slate-200 rounded-lg overflow-hidden bg-white shadow-xs">
          <table className="w-full table-fixed text-left border-collapse text-[11px]">
            <thead>
              <tr className="border-b border-black font-bold text-black leading-tight bg-slate-100">
                <th className="py-2 px-2 normal-case w-[28%]">customer name</th>
                <th className="py-2 px-2 normal-case w-[12%]">item code</th>
                <th className="py-2 px-2 normal-case w-[32%]">item description</th>
                ...
                <th className="py-2 px-2 normal-case w-[10%] text-right pr-2">total value ($)</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 font-medium text-[10.5px]">
              {defaultItems.map((row, idx) => (
                <tr key={idx} className="hover:bg-slate-50">
                  ...
                  <td className="py-2 px-2 text-right font-mono text-slate-700">{row.unitPrice}</td>
                  <td className="py-2 px-2 text-right font-mono font-bold text-emerald-800 pr-2">{row.totalValue}</td>
                </tr>
              ))}
            </tbody>
          </table>
