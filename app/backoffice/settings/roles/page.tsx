  ...
  return (
    <div dir={dir} className="p-4 sm:p-6 lg:p-8 max-w-6xl mx-auto space-y-6 animate-fadeIn font-sans">
      ...
      <div className="flex items-center gap-2 text-xs text-slate-500 font-semibold">
        ...
        <ChevronRight className="w-3.5 h-3.5 text-slate-400" />
        <span className="text-primary font-bold">{language === 'ar' ? 'الأدوار والصلاحيات' : 'Roles & Permissions'}</span>
      </div>
          ...
          <div>
            <div className="flex items-center gap-2.5">
              <h1 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight">
                {language === 'ar' ? 'مصفوفة الأدوار والصلاحيات' : 'Roles & Permissions Security Matrix'}
              </h1>
            ...
            <p className="text-xs text-slate-500 mt-1">
              {language === 'ar'
                ? 'تخصيص صلاحيات الوصول إلى المعاصر، المحاسبة، نقطة البيع، والأسطول'
                : 'Role-Based Access Control (RBAC) governance across pressing mill, POS terminals, accounting, and fleet.'}
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-3">
          <h2 className="text-xs font-extrabold text-slate-500 uppercase tracking-wider px-1">
            {language === 'ar' ? 'الأدوار المعرفة في النظام' : 'Defined System Roles'}
          </h2>
